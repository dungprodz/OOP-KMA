...
public class QuanLyKhachHang {
	...
	public static void muaHang() {
		...
		if (index == -1) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (indexSP == -1) {
					...
					if (amount < QuanLySanPham.getList().get(indexSP).getSoLuong()) {
						...
						System.out.printf("%5s %25s %3d %15.1f %10s%n", code,
								QuanLySanPham.getList().get(indexSP).getTenSP(), amount,
								QuanLySanPham.getList().get(indexSP).getDonGia(),
								QuanLySanPham.getList().get(indexSP).getNgaySX());
			...
			KhachHang kh = new KhachHang(codeKH, name, namSinh, diaChi, sum);
			list.add(kh);
		} else {
			System.out.println("nhap so loai hang can mua");
			int n = Integer.parseInt(sc.nextLine());
			...
			list.get(index).setThanhTien(sum);
		}
